...
namespace addon::atomic_flag {

struct State {
  mutable std::atomic_flag finalized;
...
void thread_func(int sleep, const State &state) {
  ...
  while (true) {
    ...
    if (state.finalized.test()) {
      fprintf(stderr, "thread_func: Finalized, exiting...\n");
      return;
    }
    auto status =
...
void tsfn_finalize(napi_env env, void *finalize_data, void *finalize_hint) {
  ...
  fprintf(stderr, "tsfn_finalize: setting tsfn to finalized\n");
  state.finalized.test_and_set();
}
